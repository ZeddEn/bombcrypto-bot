...
def getSmallDigits(img, threshold=0.95,i=0):
    ...
    if i > 10:
        if __name__ == '__main__':
            path = "./tmp/small{}.png".format(str(time.time()))
            cv2.imwrite(path,img)
            print('❌ Too many attempts for Captcha!')
            print(digits)
...
def lookForMatch(background_digits,popup_pos, has_found):
        ...
        for i in range(100):
            ...
            if small_digits == background_digits:
                pyautogui.mouseUp()
                print('✅ Captch resolved with success!', flush=True)
                has_found[0] = True
                return
...
def solveCaptcha():
    ...
    print('🧩 Captcha analyse results: {}'.format(background_digits))
